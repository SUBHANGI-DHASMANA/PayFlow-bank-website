...
app.post("/customer", wrapAsync(async(req, res, next) => {
    ...
    if (b.Balance > 0 && Amount < b.Balance && Amount > 0) {
        const f = new Transaction({ Date: Date(), amount: Amount, Description: `Recieved ₹${Amount} from ${b.username}`});
        await f.save();
        const t = new Transaction({ Date: Date(), amount: Amount, Description: `Paid ₹${Amount} to ${cus.username}`});
        await t.save()
